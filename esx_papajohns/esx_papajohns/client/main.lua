	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(vehicle)
				...
				SetVehicleNumberPlateText(vehicle, "PIZ"..platenum)             
                MissionLivraisonSelect()
				plaquevehicule = "PIZ"..platenum
...
AddEventHandler('esx_papajohns:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_papajohns:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()    
    CurrentAction = nil
    CurrentActionMsg = ''
end)
	...
	while true do
		...
		if IsJobpizza() then
			...
			if isInMarker and not hasAlreadyEnteredMarker then
				...
				TriggerEvent('esx_papajohns:hasEnteredMarker', currentZone)
			end

			if not isInMarker and hasAlreadyEnteredMarker then
				hasAlreadyEnteredMarker = false
				TriggerEvent('esx_papajohns:hasExitedMarker', lastZone)
			end
